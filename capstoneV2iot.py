...
def send_telemetry(temp, hum, press, wlm):
    msg_txt_formatted = MSG_SND.format(temperature=temp, humidity=hum, pressure=press, wlmess=wlm)
    message = Message(msg_txt_formatted)
    try:
        client.send_message(message)
        for i in range(2): #visual indicator that cmd line is moving and not broken
            print(i)
            i+=1
...
def update_labels():
    while True:
        ...
        threading.Thread(target=send_telemetry, args=(temp, hum, press, wlm), daemon=True).start()
        time.sleep(1)
